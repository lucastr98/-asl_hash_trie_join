...
void get_valid_tuples(int k, const int m, const int num_att, 
    ...
    char *join_atts[], tuple_t **result_tuples, size_t *head_index)
{
  ...
  current_tuples[j] = relation->it.list;
  while (true)
  {
    bool recurse = false;
    tuple_t *tuple_list = relation->tuple_list;
    memcpy((*result_tuples)->values + *head_index * num_att + relation->result_idx, 
        tuple_list->values + tuple_list->nFields * current_tuples[j],
        tuple_list->nFields * sizeof(char *));
    ...
    for (int i = 0; i < relation->num_join_attributes; ++i)
    {
      ...
      else
      {
        ...
        set(&mask, a);
        join_atts[a] = tuple_list->values[tuple_list->nFields * 
            current_tuples[j] + relation->tuple_indices[i]];
      }
    }
    ...
    if (recurse && (j == m - 1))
    {
      ...
      memcpy((*result_tuples)->values + (*head_index + 1) * num_att, 
          (*result_tuples)->values + (*head_index)*num_att, 
          relation->result_idx * sizeof(char *));
    ...
    // next
    current_tuples[j] = relation->tuple_list->next[current_tuples[j]];
...
void enumerate(int k, int n, int num_att, hash_trie_t *relations, 
    int m, tuple_t **result, size_t *head_index)
{
  #ifdef BENCHMARK_SUBFUNCS
  ...
  for (int relation_nr = 0; relation_nr < m; ++relation_nr)
  {
    for (int join_nr_idx = 0; 
         join_nr_idx < relations[relation_nr].num_join_attributes; 
         ++join_nr_idx)
    {
  ...
  while (true)
  {
    for (; i < n; ++i)
    {
          ...
          ++get_relation_idx)
      {
        ...
        if (node_size(relations[relation_idx].it.node) < min_node_size)
        {
          I_scan[i] = relation_idx;
          min_node_size = node_size(relations[relation_idx].it.node);
        }
      }
      #ifdef BENCHMARK_SUBFUNCS
        probe_subfunc_times.select_participants += stop_tsc(start);
           ...
           ++get_relation_idx)
      {
        ...
        if (relation_idx == I_scan[i])
          continue;
        if (!lookup(&relations[relation_idx], 
            &relations[I_scan[i]].it.node->buckets[relations[I_scan[i]].it.node->idx].hash))
        {
