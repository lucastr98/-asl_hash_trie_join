    ...
    size_t *join_atts, size_t **result_tuples, size_t *head_index)
{
  ...
  {
    bool recurse = false;
    join_tuple_t *join_tuple_list = relation->join_tuple_list;
    ...
    for (int i = 0; i < relation->num_join_attributes; ++i)
    {
      ...
      if (contains(&mask, a))
      {
        ...
        if (join_atts[a] == join_tuple_list->values[join_tuple_list->nFields * 
            current_tuples[j] + i])
        {
      ...
      else
      {
        ...
        set(&mask, a);
        join_atts[a] = join_tuple_list->values[
            join_tuple_list->nFields * current_tuples[j] + i];
      }
    }
    ...
    // next
    current_tuples[j] = relation->join_tuple_list->next[current_tuples[j]];
