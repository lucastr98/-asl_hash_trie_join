    ...
    const char *sql_options, const bool is_validation)
{
  ...
  for (int j = 0; j < benchmark->num_relations; ++j)
  {
    ...
    tuple_t *head1 = malloc(sizeof(*head1) + nFields * sizeof(*head1->values));
    head1->next = NULL;
    for (int k = 0; k < nFields; k++)
    {
      char *s = PQgetvalue(res, 0, k);
      size_t len = strlen(s);
      head1->values[k] = malloc((len + 1) * sizeof(char));
      strncpy(head1->values[k], s, len + 1);
    }
    tuple_t *tmp = head1;
    for (int k = 1; k < nTuples; k++)
    {
      ...
      for (int l = 0; l < nFields; l++)
      {
        ...
        size_t len = strlen(s);
        tup->values[l] = malloc((len + 1) * sizeof(char));
        strncpy(tup->values[l], s, len + 1);
      }
      tmp->next = tup;
      tmp = tup;
    }
    
    benchmark->relations[j].tuple_list = head1;
